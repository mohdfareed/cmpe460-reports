...
#define INTEGRATION_TIME .04f // use 50Hz (20ms) for now
...
void ControlPin_SI_Init()
{
	...
	P5->SEL0 &= ~SI;
	P5->SEL1 &= ~SI; // set as GPIO
	P5->DIR |= SI;	 // set as output
	P5->OUT &= ~SI;	 // set low
...
void ControlPin_CLK_Init()
{
	...
	unsigned long period = CalcPeriodFromFrequency(400000);

	// initialize P5.4 and make it output (P5.4 CLK Pin)
	P5->SEL0 &= ~CLK;
	P5->SEL1 &= ~CLK; // set as GPIO
	P5->DIR |= CLK;	  // set as output
	P5->OUT &= ~CLK;  // set low
